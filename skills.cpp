...
void autonomous() {
    ...
    // Task to run the conveyor and pre-roller
    pros::Task conveyorTask(moveConveyerTask);

    // Phase 1: Grab the first mobile goal and score rings
    chassis.moveToPose(0, -32, 0, 1100, {.forwards = false, .maxSpeed = 80});
    pros::delay(850);
    mogo.set_value(true); // Grab the mobile goal
    ...
    pros::delay(300);
    preRoller.move(127); // Start pre-roller
    conv.move(127); // Start conveyor
    ...
    // Phase 2: Score rings in the middle
    chassis.moveToPose(-22, 0, -90, 1600, {.maxSpeed = 110});
    chassis.moveToPose(-38, 0, -90, 1200, {.maxSpeed = 80});
    pros::delay(100);
    chassis.turnToHeading(50, 750);
    pros::delay(300);
    conv.move(0); // Stop conveyor
    mogo.set_value(false); // Release mobile goal
    preRoller.move(0); // Stop pre-roller
    chassis.moveToPose(-29, 4, 90, 1500, {.maxSpeed = 100});

    // Phase 3: Put the last ring on the wall stake
    chassis.turnToHeading(175, 750);
    correctPosition(-29, 4); // Ensure accurate positioning
    chassis.moveToPoint(-29, 21, 1000, {.forwards = false, .maxSpeed = 60});
    pros::delay(600);
    conv.move(127); // Score the last ring
    pros::delay(1000);
    conv.move(0); // Stop conveyor

    // Phase 4: Return to collect more rings
    chassis.moveToPoint(-29, -28, 2000, {.maxSpeed = 40});
    pros::delay(200);

    // Phase 5: Collect and score additional rings
    chassis.turnToHeading(-315, 1500);
    chassis.moveToPoint(0, -33, 2000, {.forwards = true, .maxSpeed = 30});
    preRoller.move(127); // Start pre-roller
    conv.move(127); // Start conveyor
    pros::delay(500);
    chassis.moveToPose(10, -40, -270, 1500, {.maxSpeed = 80});
    pros::delay(300);
    chassis.moveToPose(20, -40, -270, 1500, {.maxSpeed = 80});

    // Phase 6: Score additional rings
    chassis.moveToPose(-22, 0, -90, 1600, {.maxSpeed = 110});
    chassis.moveToPose(-38, 0, -90, 1200, {.maxSpeed = 80});
    pros::delay(100);
    chassis.turnToHeading(50, 750);
    ...
    // Phase 7: Final scoring and repositioning
    chassis.moveToPose(-29, 4, 90, 1500, {.maxSpeed = 100});
    chassis.turnToHeading(175, 750);
    correctPosition(-29, 4);
    chassis.moveToPoint(-29, 21, 1000, {.forwards = false, .maxSpeed = 60});
    pros::delay(600);
    conv.move(127); // Score the final ring
    pros::delay(1000);
    conv.move(0); // Stop conveyor
}
